...
int main(int argc, char **argv)
{
    ...
    std::string filePath = "mandelbrot.cu";
    ...
    std::string mandelStr = buffer.str() + "\n";

    // Create an instance of nvrtcProgram with the MANDEL code string.
    nvrtcProgram prog;
    NVRTC_SAFE_CALL(
        nvrtcCreateProgram(&prog,           // prog
        mandelStr.c_str(),                  // buffer
        "mandel.cu",                        // name
        0,                                  // numHeaders
        NULL,                               // headers
        NULL));                             // includeNames
    ...
    CUDA_SAFE_CALL(cuModuleGetFunction(&cuKernel, cuModule, "mandelbrot_k"));
    ...
    // Execute MANDELBROT.
    std::cout << "Running mandelbrot on " << W << " x " << H << " elements" << std::endl;
    double t1 = omp_get_wtime();
    
    void *args[] = { &dOut, &w, &h, &cmin, &cmax };
    CUDA_SAFE_CALL(
    ...
    //    h_dwells[i] = pixel;
    //}
    //savePPM("mandel", (unsigned char*)h_dwells, w, h, 4);
